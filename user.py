...
from schemas import AddAudioSchema, AddFavouriteSchema, CreateShopSchema, GetCartSchema, GetFavouriteSchema, GetHomepage, GuestQuerySchema, HomepageSchema, PlaceOrderSchema, PlanStatusQuerySchema, RemoveFavouriteSchema, RestaurantDetailSchema, SignupSchema, SignupQuerySchema, LoginQuerySchema, LoginSchema, SubscribeQuerySchema, SubscribeSchema, SuccessMessageSchema, UserDeleteSchema, UserDetailSchema, UserListSchema, UserLoginSchema, UserLogoutSchema, UserSignupSchema, VisitorLoginQuerySchema
import hashlib
...
class UserLogin(MethodView):
    ...
    # @blp.response(200, GetHomepage)
    @blp.arguments(GetFavouriteSchema, location="query")
    def get(self, request_data):
        query = request_data['user_id']
        result = self.db.get_favourite(query)
        if result is None:
            abort(404, message="Page Not Found")
        return result, 200
    
    @blp.arguments(AddFavouriteSchema)
    def post(self, request_data):
        #request_data = request.get_json()
        #print(request_data)
        parentid = request_data['parentid']
        user_id = request_data['user_id']
        result = self.db.add_favourite(parentid,user_id)
        if result is None:
            abort(400, message="Username or password is incorrect")
        return {"message": "Audio added to favourite succesfully"}, 201
